...
namespace Odb::Lib::ProductModel
{
	...
	// UTF-8 key only through the five undefined CP1252 slots (0x81, 0x8D, 0x8F,
	// 0x90, 0x9D), which all map to U+FFFD. When that happens the first entry
	// wins and the collision is logged, so no entry is ever silently overwritten.
