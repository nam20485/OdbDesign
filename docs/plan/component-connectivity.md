...
The surface stays live and swagger-published but takes no new endpoints; fields added to the shared message ride along on every flavour (that is what D4 decides), and no new REST *surface* is built. Concretely: `designs_component_route_handler` (`DesignsController.cpp:311`) and `designs_net_route_handler` (`:343`) stay unimplemented, registered routes commented out. **Deleting them was proposed and deferred by the owner on 2026-09-15** — they are not a security concern, so removing them is out of scope under a freeze; they stay as harmless dead scaffolding until the REST surface itself is revisited. The existing collection routes stay as-is. **M4.2 is deferred, not executed**: deleting the stubs is not a security fix, so it stays out of scope under this freeze.
...
- Additive only in practice: never renumber, never reuse a tag, never make an absent field ambiguous. Everything in this plan conforms — Phase 1 changes no `.proto` **except** for M1.4, whose recommended option deletes the never-populated `componentRecordsByName` (field 10) — non-additive, but wire-safe precisely because the field is always empty, and it owes each client repo a push PR under this rule. Everything else in Phase 1 is schema-neutral (`optional uint32 id = 9` already exists and was simply never assigned; removing the `"ID"` map key changes values inside a `map<string,string>`, not its type). Phase 2 adds one field, `connectivity = 12`.
