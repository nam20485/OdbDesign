...
Verified 2026-09-15, so nobody re-investigates this as a build-time win: codegen is already incremental. Each proto gets its own ninja edge depending only on that `.proto`, with `restat = 1`, so a dry run on a current tree requests zero `protoc` runs and zero `*.pb.cc` compiles, and editing one proto regenerates only that pair. No generated source is tracked (`git ls-files | grep -c '\.pb\.'` = 0). **Do not commit a gencode copy** — it would be a build-input projection of a source of truth, the same failure shape as the ConfigMap that drifted 1,063 lines in #594, and it interacts badly with the known requirement to wipe and reconfigure after a protobuf major bump. Adding `connectivity.proto` is the only real sharp edge: it needs a reconfigure, for the reason above.
