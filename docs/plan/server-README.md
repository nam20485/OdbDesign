...
**From:** the OdbDesign 3D client team · **Date:** 2026-09-10 · **Note:** written in the *client* repo and committed into the server repo on 2026-09-25 via #595 — "this repo" in the original meant the client repo, and the option/status files cited below live there, not here.
...
| gRPC TLS | Secure the gRPC channel (currently plaintext `:50051` behind Tailscale) | Open — options in `grpc-auth-options-and-dev-plan.md` (client repo; not present here) |
| Unified auth | One mechanism beyond HTTP Basic, covering gRPC + REST: per-user/per-resource authorization, interactive + device flow | Open — options in `grpc-auth-options-and-dev-plan.md` (client repo; not present here) |
...
| gRPC perf phases 2–4 | Arena allocation, compression tuning, and the rest of the Feb optimization plan | Open — full file-by-file status in `server-grpc-optimization-remaining-work.md` (client repo; not present here) (includes an unmerged `opt-phase2` branch that already implements S-1..S-3) |
